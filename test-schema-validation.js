...
async function runFullValidation() {
  ...
  serviceResults.forEach(result => {
    if (result.error) {
      ...
      log(`${result.valid ? '✓' : '✗'} ${result.page}: Schema=${result.hasSchema}, Breadcrumbs=${result.hasBreadcrumbs}, ServiceName=${result.hasServiceName}`);
    }
  });
  ...
  if (reviewValidation.error) {
    ...
    log(`${reviewValidation.valid ? '✓' : '✗'} Reviews: Count=${reviewValidation.reviewCount}, Valid=${reviewValidation.valid}`);
