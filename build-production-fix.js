...
async function main() {
  try {
    ...
    execSync(
      `npx esbuild client/src/main.tsx --bundle --outfile=dist/public/assets/main.js --format=esm --target=es2020 --jsx=automatic --jsx-import-source=react --loader:.tsx=tsx --loader:.ts=tsx --loader:.css=css --define:process.env.NODE_ENV='"production"' --minify`,
      {
        stdio: 'inherit',
      }
    );
    ...
    log('Building server...');
    execSync(
      'npx esbuild server/index.ts --platform=node --packages=external --bundle --format=esm --outdir=dist --define:process.env.NODE_ENV=\'"production"\'',
      {
        stdio: 'inherit',
      }
    );
